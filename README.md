...
## المتطلبات:
- Node.js 20.19.0 أو أحدث (تأكدي بالأمر: node -v)
- ملف `.env` لازم يحتوي `JWT_SECRET` (موجود بـ `.env.example`) — بدونه التطبيق ما رح يشتغل ورح يطلع خطأ واضح بدل ما يفشل بصمت.
...
- `register` هلق ملفوف بـ `prisma.$transaction` (إنشاء الـ User والـ Patient سوا) — لو فشلت خطوة إنشاء الـ Patient لأي سبب، ما بيضل عندنا User يتيم بدون ملف مريض عاجز عن الحجز.
- `.env` مش مرفوعة على git إطلاقاً (بس `.env.example`)، و`dev.db` كمان مستثناة بالكامل — قاعدة البيانات وملفات الأسرار محلية فقط، الكود والبنية بس يلي مشترك.
